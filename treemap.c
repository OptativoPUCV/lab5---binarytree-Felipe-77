...
Pair * upperBound(TreeMap * tree, void* key) {
    ...
    while (current != NULL){
        ...
        if (tree->lower_than(result->key, current->pair->key) == 0){
            if (tree->lower_than(key, current->pair->key) == 1 || is_equal(tree, key, current->pair->key) == 1){
                result = current->pair;
            }
        }
        if (tree->lower_than(key, current->pair->key)){
...
Pair * nextTreeMap(TreeMap * tree) {
    if (tree->current->right == NULL){
        ...
        if (tree->current->parent->parent != NULL && tree->lower_than(tree->current->pair->key, tree->current->parent->parent->pair->key) == 1){
